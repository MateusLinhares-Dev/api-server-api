...
export async function transcriptionbeAudio(
  ...
  const response = await gemini.models.generateContent({
    ...
    contents: [
      {
        text: 'Transcreva o áudio para português do Brasil. Seja preciso e natural na transcrição. Mantenha a pontuação adequada e divida o texto em parágrafos quando for apropriada.',
      },
      {
        inlineData: {
          mimeType,
  ...
  if (!response.text) {
    throw new Error(`Error transcribing audio: ${response.error.message}`);
  }
